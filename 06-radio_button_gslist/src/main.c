...
void on_bt_confirm_clicked (GtkButton *bt_confirm, void *data)
{
    ...
    GSList *rb_list = gtk_radio_button_get_group (GTK_RADIO_BUTTON (w->rb_white));
    GtkToggleButton *rb = NULL;
    ...
    while (rb_list)
    {
        ...
        {
            GtkWidget *label = gtk_bin_get_child (GTK_BIN (rb));
            const gchar *text = gtk_label_get_text (GTK_LABEL (label));
            snprintf (buffer, 1024, template, text);
            break;
        }
